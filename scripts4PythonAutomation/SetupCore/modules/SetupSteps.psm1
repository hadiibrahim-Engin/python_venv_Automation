...
function Invoke-PrecheckStep {
    ...
    if (-not $prechecks.ContinueSetup) {
        if ($DryRun) {
            ...
            throw (New-SetupException -Message 'Setup aborted: critical prechecks failed.' -ErrorCode 'PRECHECK_FAILED' -Step 'PRECHECK' -Context @{ ProjectRoot = $Ctx.ProjectRoot })
        }
    }
    $prechecks
}
...
function Invoke-PackageManagerSigningStep {
    ...
    $result = Set-PoetryShimSignature -ShimPath $pmShimPath
...
function Invoke-SmartSigningStep {
    ...
    $Ctx.SignResult = Set-VenvScriptSignature -VenvDir $Ctx.VenvDir -PoetryOnly:$SignPoetryOnly
    $Ctx.SignResult
}

function Invoke-StaleCleanupStep {
    [CmdletBinding()] param([Parameter(Mandatory=$true)][hashtable] $Ctx)
    Remove-StaleQuarantines -ProjectRoot $Ctx.ProjectRoot
}
...
#>
    ...
    switch ($Ctx.PackageManager) {
        'uv' {
            $uvExe = Get-UvExe
            if (-not $uvExe) { throw 'update-venv requires uv, but uv is not installed or discoverable.' }
        ...
        'pip-requirements' {
            $args = @('-m','pip','install')
            if ($Ctx.UpdateDependencies -and -not $Ctx.PinExact) { $args += '--upgrade' }
        ...
        default {
            if (Test-Path -LiteralPath $requirementsFile -PathType Leaf) {
                Invoke-NativeCommand -Executable $venvPython -Arguments @('-m','pip','install','-r',$requirementsFile) -WorkingDirectory $Ctx.ProjectRoot -PassThrough -ThrowOnError | Out-Null
            } elseif (Test-Path -LiteralPath $pyprojectFile -PathType Leaf) {
                Invoke-NativeCommand -Executable $venvPython -Arguments @('-m','pip','install','-e',$Ctx.ProjectRoot) -WorkingDirectory $Ctx.ProjectRoot -PassThrough -ThrowOnError | Out-Null
    ...
    # Important: update mode must not blindly re-sign every binary. The signing
    # module first validates Authenticode and only sends new/changed/invalid files
    # to DigiCert.
