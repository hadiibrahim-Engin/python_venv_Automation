...
PowerShell automation for creating, repairing, signing, and activating a Python virtual environment from a project root. The toolkit supports both `uv` and Poetry, can discover or validate a local Python interpreter, requires DigiCert Utility, and is designed to be safe to run repeatedly.
...
- Windows plus DigiCert Utility for a real setup run
...
DigiCert is treated as required infrastructure, not a best-effort enhancement. If DigiCert Utility is missing, prechecks abort before setup mutates the environment.
...
12. Copy Tcl runtime files when present.
13. Code-sign executables and clean up backups.
...
If you want a clean rebuild, use `-ForceRecreateVenv`. If you want to see what would happen without modifying anything, use `-DryRun`; dry-run reports required precheck failures but continues printing the remaining plan.
